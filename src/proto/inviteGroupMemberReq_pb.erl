...
-module(inviteGroupMemberReq_pb).
...
-include("inviteGroupMemberReq_pb.hrl").
-include("gpb.hrl").
...
-type inviteGroupMemberReq() :: #inviteGroupMemberReq{}.
-export_type(['groupMember'/0, 'inviteGroupMemberReq'/0]).
...
encode_msg(Msg, Opts) ->
    ...
    case Msg of
      ...
      #inviteGroupMemberReq{} ->
	  e_msg_inviteGroupMemberReq(Msg, TrUserData)
    end.
...
e_msg_inviteGroupMemberReq(#inviteGroupMemberReq{members
						     = F1},
			   Bin, TrUserData) ->
    begin
      TrF1 = id(F1, TrUserData),
      if TrF1 == [] -> Bin;
	 true ->
	     e_field_inviteGroupMemberReq_members(TrF1, Bin,
						  TrUserData)
      end
    end.

e_mfield_inviteGroupMemberReq_members(Msg, Bin,
				      TrUserData) ->
    SubBin = e_msg_groupMember(Msg, <<>>, TrUserData),
    Bin2 = e_varint(byte_size(SubBin), Bin),
    <<Bin2/binary, SubBin/binary>>.

e_field_inviteGroupMemberReq_members([Elem | Rest], Bin,
				     TrUserData) ->
    Bin2 = <<Bin/binary, 10>>,
    Bin3 = e_mfield_inviteGroupMemberReq_members(id(Elem,
						    TrUserData),
						 Bin2, TrUserData),
    e_field_inviteGroupMemberReq_members(Rest, Bin3,
					 TrUserData);
e_field_inviteGroupMemberReq_members([], Bin,
				     _TrUserData) ->
    Bin.
...
decode_msg(Bin, MsgName, Opts) when is_binary(Bin) ->
    ...
    case MsgName of
      ...
      inviteGroupMemberReq ->
	  d_msg_inviteGroupMemberReq(Bin, TrUserData)
    end.
...
d_msg_inviteGroupMemberReq(Bin, TrUserData) ->
    dfp_read_field_def_inviteGroupMemberReq(Bin, 0, 0,
					    id([], TrUserData), TrUserData).

dfp_read_field_def_inviteGroupMemberReq(<<10,
					  Rest/binary>>,
					Z1, Z2, F1, TrUserData) ->
    d_field_inviteGroupMemberReq_members(Rest, Z1, Z2, F1,
					 TrUserData);
dfp_read_field_def_inviteGroupMemberReq(<<>>, 0, 0, F1,
					TrUserData) ->
    #inviteGroupMemberReq{members =
			      lists_reverse(F1, TrUserData)};
dfp_read_field_def_inviteGroupMemberReq(Other, Z1, Z2,
					F1, TrUserData) ->
    dg_read_field_def_inviteGroupMemberReq(Other, Z1, Z2,
					   F1, TrUserData).

dg_read_field_def_inviteGroupMemberReq(<<1:1, X:7,
					 Rest/binary>>,
				       N, Acc, F1, TrUserData)
    when N < 32 - 7 ->
    dg_read_field_def_inviteGroupMemberReq(Rest, N + 7,
					   X bsl N + Acc, F1, TrUserData);
dg_read_field_def_inviteGroupMemberReq(<<0:1, X:7,
					 Rest/binary>>,
				       N, Acc, F1, TrUserData) ->
    Key = X bsl N + Acc,
    case Key of
      10 ->
	  d_field_inviteGroupMemberReq_members(Rest, 0, 0, F1,
					       TrUserData);
      _ ->
	  case Key band 7 of
	    0 ->
		skip_varint_inviteGroupMemberReq(Rest, 0, 0, F1,
						 TrUserData);
	    1 ->
		skip_64_inviteGroupMemberReq(Rest, 0, 0, F1,
					     TrUserData);
	    2 ->
		skip_length_delimited_inviteGroupMemberReq(Rest, 0, 0,
							   F1, TrUserData);
	    5 ->
		skip_32_inviteGroupMemberReq(Rest, 0, 0, F1, TrUserData)
	  end
    end;
dg_read_field_def_inviteGroupMemberReq(<<>>, 0, 0, F1,
				       TrUserData) ->
    #inviteGroupMemberReq{members =
			      lists_reverse(F1, TrUserData)}.

d_field_inviteGroupMemberReq_members(<<1:1, X:7,
				       Rest/binary>>,
				     N, Acc, F1, TrUserData)
    when N < 57 ->
    d_field_inviteGroupMemberReq_members(Rest, N + 7,
					 X bsl N + Acc, F1, TrUserData);
d_field_inviteGroupMemberReq_members(<<0:1, X:7,
				       Rest/binary>>,
				     N, Acc, F1, TrUserData) ->
    Len = X bsl N + Acc,
    ...
    NewFValue = id(d_msg_groupMember(Bs, TrUserData),
		   TrUserData),
    dfp_read_field_def_inviteGroupMemberReq(Rest2, 0, 0,
					    cons(NewFValue, F1, TrUserData),
					    TrUserData).


skip_varint_inviteGroupMemberReq(<<1:1, _:7,
				   Rest/binary>>,
				 Z1, Z2, F1, TrUserData) ->
    skip_varint_inviteGroupMemberReq(Rest, Z1, Z2, F1,
				     TrUserData);
skip_varint_inviteGroupMemberReq(<<0:1, _:7,
				   Rest/binary>>,
				 Z1, Z2, F1, TrUserData) ->
    dfp_read_field_def_inviteGroupMemberReq(Rest, Z1, Z2,
					    F1, TrUserData).


skip_length_delimited_inviteGroupMemberReq(<<1:1, X:7,
					     Rest/binary>>,
					   N, Acc, F1, TrUserData)
    when N < 57 ->
    skip_length_delimited_inviteGroupMemberReq(Rest, N + 7,
					       X bsl N + Acc, F1, TrUserData);
skip_length_delimited_inviteGroupMemberReq(<<0:1, X:7,
					     Rest/binary>>,
					   N, Acc, F1, TrUserData) ->
    Length = X bsl N + Acc,
    <<_:Length/binary, Rest2/binary>> = Rest,
    dfp_read_field_def_inviteGroupMemberReq(Rest2, 0, 0, F1,
					    TrUserData).


skip_32_inviteGroupMemberReq(<<_:32, Rest/binary>>, Z1,
			     Z2, F1, TrUserData) ->
    dfp_read_field_def_inviteGroupMemberReq(Rest, Z1, Z2,
					    F1, TrUserData).


skip_64_inviteGroupMemberReq(<<_:64, Rest/binary>>, Z1,
			     Z2, F1, TrUserData) ->
    dfp_read_field_def_inviteGroupMemberReq(Rest, Z1, Z2,
					    F1, TrUserData).
...
merge_msgs(Prev, New, Opts)
    ...
    case Prev of
      ...
      #inviteGroupMemberReq{} ->
	  merge_msg_inviteGroupMemberReq(Prev, New, TrUserData)
    end.
...
merge_msg_inviteGroupMemberReq(#inviteGroupMemberReq{members
							 = PFmembers},
			       #inviteGroupMemberReq{members = NFmembers},
			       TrUserData) ->
    #inviteGroupMemberReq{members =
...
verify_msg(Msg, Opts) ->
    ...
    case Msg of
      ...
      #inviteGroupMemberReq{} ->
	  v_msg_inviteGroupMemberReq(Msg, [inviteGroupMemberReq],
				     TrUserData);
...
-dialyzer({nowarn_function,v_msg_inviteGroupMemberReq/3}).
v_msg_inviteGroupMemberReq(#inviteGroupMemberReq{members
						     = F1},
			   Path, TrUserData) ->
    if is_list(F1) ->
...
find_msg_def(groupMember) ->
    [#field{name = groupId, fnum = 1, rnum = 2,
     ...
     #field{name = nickName, fnum = 3, rnum = 4,
	    type = string, occurrence = optional, opts = []}];
find_msg_def(inviteGroupMemberReq) ->
    [#field{name = members, fnum = 1, rnum = 2,
	    type = {msg, groupMember}, occurrence = repeated,
	    opts = []}];
